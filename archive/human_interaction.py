...
class HumanInteractionController:
    ...
    def _move_mouse_human_like(self, x: int, y: int):
        """
        Déplace la souris vers une cible en simulant une courbe de Bézier, 
        une vitesse humaine et un potentiel dépassement (overshoot).
        """
        ...
        # Points de controle pour Bezier quadratique
        cp_x = current_x + (target_x - current_x) * random.uniform(0.3, 0.7) + random.randint(-100, 100)
        cp_y = current_y + (target_y - current_y) * random.uniform(0.3, 0.7) + random.randint(-100, 100)
        ...
        for i in range(1, steps + 1):
            t = i / steps
            # Bezier Quadratique
            bx = ((1 - t) ** 2) * current_x + 2 * (1 - t) * t * cp_x + (t ** 2) * target_x
            by = ((1 - t) ** 2) * current_y + 2 * (1 - t) * t * cp_y + (t ** 2) * target_y
            pyautogui.moveTo(bx, by)
            time.sleep(duration / steps)
            
        # Overshoot (dépassement léger) 40% du temps
        if random.random() < 0.40:
            ox = target_x + random.randint(-15, 15)
            oy = target_y + random.randint(-15, 15)
            pyautogui.moveTo(ox, oy, duration=random.uniform(0.08, 0.15), tween=self._ease_out_quad)
            pyautogui.moveTo(target_x, target_y, duration=random.uniform(0.08, 0.15), tween=self._ease_out_quad)
            
        self._human_sleep(0.05)
    ...
    def execute_action(self, action: str, amount: float = None, coords: dict = None):
        ...
        # Temps de réaction avant de bouger la souris (proportionnel à la complexité)
        if action == "FOLD":
            react_time = random.uniform(1.0, 2.5)
        elif action in ["CHECK", "CALL"] and amount is None:
            react_time = random.uniform(2.0, 4.5)
        else:
            react_time = random.uniform(4.0, 12.0)
            
        self._human_sleep(react_time) 
