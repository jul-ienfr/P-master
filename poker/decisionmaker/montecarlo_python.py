...
def _run_montecarlo_wrapper_legacy(p, ui_action_and_signals, config, ui, t, L, preflop_state, h):
...
def run_montecarlo_wrapper(p, ui_action_and_signals, config, ui, t, L, preflop_state, h):
    logger = logging.getLogger('montecarlo')
    try:
        from .native_equity import run_native_equity_wrapper

        return run_native_equity_wrapper(
            p,
        ...
        logger.warning("Native equity unavailable, falling back to Python Monte Carlo: %s", exc)
        return _run_montecarlo_wrapper_legacy(
            p,
            ui_action_and_signals,
            config,
            ui,
