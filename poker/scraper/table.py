...
class Table(TableScraper):
    ...
    def take_screenshot(self, initial, p):
        if initial:
            ...
            time.sleep(0.1)
        if hasattr(self, 'preloaded_entire_screen_pil') and self.preloaded_entire_screen_pil is not None:
            self.entireScreenPIL = self.preloaded_entire_screen_pil
            self.screenshot = self.preloaded_entire_screen_pil
            self.preloaded_entire_screen_pil = None
        else:
            config = get_config()
