...
from poker.gui.i18n import apply_translations, get_language, translate_text
...
from poker.tools.supported_sites import SUPPORTED_SITES
...
class TableSetupActionAndSignals(QObject):
    ...
    def __init__(self, ui):
        ...
        self.setup_room_manager_toolbar()
    ...
    def setup_room_manager_toolbar(self):
        try:
            toolbar = QtWidgets.QToolBar("Room Manager", self.ui)
            toolbar.setObjectName("room_manager_toolbar")
            self.ui.addToolBar(Qt.ToolBarArea.TopToolBarArea, toolbar)
        except Exception:
            return

        actions = [
            ("Wizard", self.start_room_wizard, "Create or clone a room preset with metadata."),
            ("Validate", self.validate_current_preset, "Run replay validation on the current draft."),
            ("Publish", self.publish_current_preset, "Publish a new immutable local version."),
            ("History", self.show_preset_history, "Inspect published versions and status."),
            ("Rollback", self.rollback_current_preset, "Reactivate a previous version instantly."),
            ("Drift", self.run_drift_check, "Run drift detection against the latest screenshot."),
            ("Sync", self.sync_current_preset, "Push the active local preset to the legacy remote API."),
        ]
        for label, callback, tooltip in actions:
            action = QtGui.QAction(label, self.ui)
            action.setToolTip(tooltip)
            action.triggered.connect(callback)
            toolbar.addAction(action)

    def refresh_table_names(self, selected=None):
        selected = selected or self.ui.table_name.currentText()
        tables = mongo.get_available_tables(COMPUTER_NAME)
        self.ui.table_name.blockSignals(True)
        self.ui.table_name.clear()
        self.ui.table_name.addItems(tables)
        if selected in tables:
            self.ui.table_name.setCurrentText(selected)
        elif tables:
            self.ui.table_name.setCurrentIndex(0)
        self.ui.table_name.blockSignals(False)

    def ensure_live_screenshots(self):
        if not self.screenshot_list:
            self.take_screenshot()
        return list(self.screenshot_list)

    def start_room_wizard(self):
        preset_name, ok = QtWidgets.QInputDialog.getText(
            self.ui,
            "Room Wizard",
            "New preset name:",
            text=self.ui.new_name.text() or self.ui.table_name.currentText() or "New Room Preset",
        )
        if not ok or preset_name.strip() == "":
            return
        preset_name = preset_name.strip()

        clone_sources = ["Blank"] + mongo.get_available_tables(COMPUTER_NAME)
        clone_from, ok = QtWidgets.QInputDialog.getItem(
            self.ui,
            "Room Wizard",
        ...
        if clone_from == "Blank":
            created = mongo.create_new_table(preset_name)
        else:
            created = mongo.create_new_table_from_old(preset_name, clone_from)

        if not created:
            pop_up("Unable to create preset", "Please choose a different preset name.")
            return

        site_options = [site.display_name for site in SUPPORTED_SITES] + ["Custom"]
        site_choice, ok = QtWidgets.QInputDialog.getItem(
            self.ui,
            "Room Wizard",
            "Site family:",
            site_options,
        ...
        if not ok or variant.strip() == "":
            variant = "default"

        theme, ok = QtWidgets.QInputDialog.getText(
            self.ui,
            "Room Wizard",
            "Theme / skin:",
            text=site_choice if site_choice != "Custom" else preset_name,
        )
        if not ok or theme.strip() == "":
            theme = site_choice if site_choice != "Custom" else preset_name

        table_kind, ok = QtWidgets.QInputDialog.getItem(
            self.ui,
            "Room Wizard",
            "Cash or tournament:",
            ["cash", "tournament"],
        ...
        real_or_play, ok = QtWidgets.QInputDialog.getItem(
            self.ui,
            "Room Wizard",
            "Real or play money:",
            ["real", "play"],
        ...
        identity = {
            "site": site_choice.lower().replace(" ", "_"),
            "network": site_choice,
            "skin": theme,
            "variant": variant,
            "max_players": int(self.ui.max_players.currentText() or "6"),
            "theme": theme,
            "table_size": "default",
            "hero_seat": "bottom-right",
            "cash_or_tournament": table_kind,
            "real_or_play": real_or_play,
        }
        mongo.update_table_identity(preset_name, identity)
        self.refresh_table_names(selected=preset_name)
        self.ui.table_name.setCurrentText(preset_name)

        ai_suggestion = mongo.suggest_table_with_ai(preset_name, screenshots=self.screenshot_list)
        details = "\n".join(ai_suggestion.get("notes", [])) or "No local suggestions yet."
        pop_up(
            "Room Wizard ready",
            (
                f"Preset '{preset_name}' is ready.\n\n"
                "Next steps:\n"
                "1. Take screenshots\n"
                "2. Save top-left corner\n"
                "3. Crop\n"
                "4. Map search areas and templates\n"
                "5. Validate\n"
                "6. Publish\n"
            ),
            details=details,
    ...
    def validate_current_preset(self):
        table_name = self.ui.table_name.currentText()
        if table_name == "":
            return
        screenshots = self.ensure_live_screenshots()
        result = mongo.validate_table(table_name, screenshots=screenshots, use_draft=True)
        pop_up(
            "Validation result",
            (
                f"Status: {result.status}\n"
                f"Golden pass rate: {result.golden_pass_rate:.2f}\n"
                f"Live pass rate: {result.live_pass_rate:.2f}\n"
                f"Critical anchor score: {result.critical_anchor_score:.2f}"
            ),
            details="\n".join(result.issues) if result.issues else "No blocking issues detected.",
        )
    ...
    def publish_current_preset(self):
        ...
        screenshots = self.ensure_live_screenshots()
        try:
            result = mongo.publish_table_draft(table_name, screenshots=screenshots)
        except RuntimeError as exc:
            pop_up("Publish failed", str(exc))
            return
        pop_up(
            "Preset published",
            (
                ...
                f"Status: {result['status']}\n"
                f"Active version: {result['active_version_id']}"
            ),
        )

    def show_preset_history(self):
        table_name = self.ui.table_name.currentText()
        if table_name == "":
            return
        versions = mongo.list_table_versions(table_name)
        if not versions:
            pop_up("No history", "No published versions are available yet.")
            return
        lines = [
            f"{version['version_id']} | {version['status']} | {version['published_at']} | score={version['score']}"
            for version in versions
        ]
        details = "\n".join(lines)
        if len(versions) >= 2:
            comparison = mongo.compare_table_versions(
                table_name,
                versions[-1]["version_id"],
                versions[-2]["version_id"],
            )
            details += "\n\nLatest comparison:\n"
            details += f"Changed keys: {', '.join(comparison['changed_keys']) or 'None'}\n"
            details += f"Changed assets: {', '.join(comparison['changed_assets']) or 'None'}"
        pop_up("Preset history", f"{len(versions)} published version(s).", details=details)

    def rollback_current_preset(self):
        table_name = self.ui.table_name.currentText()
        if table_name == "":
            return
        versions = mongo.list_table_versions(table_name)
        if not versions:
            pop_up("No rollback target", "This preset has no published versions yet.")
            return
        choices = [version["version_id"] for version in versions]
        version_id, ok = QtWidgets.QInputDialog.getItem(
            self.ui,
            "Rollback preset",
            "Select the version to reactivate:",
            choices,
            editable=False,
        )
        if not ok or version_id == "":
            return
        mongo.rollback_table_version(table_name, version_id)
        pop_up("Rollback complete", f"{table_name} now points to version {version_id}.")
    ...
    def run_drift_check(self):
        ...
        result = mongo.observe_runtime_table(table_name, screenshot)
        pop_up(
            "Drift detection",
            f"Status: {result.status}\nScore: {result.score:.2f}\nVersion: {result.version_id or 'none'}",
            details="\n".join(result.diagnostics),
        )

    def sync_current_preset(self):
        table_name = self.ui.table_name.currentText()
        if table_name == "":
            return
        synced = mongo.sync_table_to_remote(table_name)
        if synced:
            pop_up("Remote sync", f"{table_name} was exported to the legacy remote API.")
        else:
            pop_up("Remote sync", "Remote sync is unavailable or failed.")
    ...
    def blank_new(self):
        ...
        if ok:
            self.refresh_table_names(selected=self.ui.new_name.text())
        else:
    ...
    def take_screenshot_timed(self):
        if not self.is_recording:
            ...
            self.is_recording = False
            self.timer.stop()
            self.ui.take_screenshot_button.setText(translate_text("Take screenshot", self.current_language))
            return
    ...
    def take_screenshot_guard(self):
        ...
        if len(self.screenshot_list) == self.x_times:
            ...
            self.take_screenshot()
            self.ui.take_screenshot_button.setText(
                translate_text("Cancel (" + str(self.x_times + 1 - len(self.screenshot_list)) + ")", self.current_language)
            )
        else:
...
def pop_up(title, text, details=None, ok_cancel=False):
    ...
    msg = QMessageBox()
    msg.setWindowTitle(translate_text(title, language))
    msg.setText(translate_text(text, language))
    if ok_cancel:
        msg.setStandardButtons(QMessageBox.StandardButton.Ok | QMessageBox.StandardButton.Cancel)
    if details:
        msg.setDetailedText(translate_text(details, language))
    response = msg.exec()
    log.info(response)
    return response
