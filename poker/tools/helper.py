...
def init_logger(screenlevel, filename=None, logdir=None, modulename=''):
    ...
    screenshots_path = "log/screenshots"
    try:
        if not os.path.exists(screenshots_path):
            os.makedirs(screenshots_path)
    except OSError:
        print(f"Creation of the directory '{screenshots_path}' failed")
        exit(1)
    else:
        print(f"Successfully created the directory '{screenshots_path}' ")
...
def _keys_to_tuple(args, kwargs):
    ...
    compiled_args = []
    for arg in args:
        if isinstance(arg, dict) or (pd is not None and isinstance(arg, pd.DataFrame)):
