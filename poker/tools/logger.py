...
def init_logger(screenlevel, filename):
    ...
    fh2 = logging.handlers.RotatingFileHandler(
        os.path.join(logdir, filename + ".log"), maxBytes=300000, backupCount=20
    )
    fh2.setLevel(logging.INFO)
    er = logging.handlers.RotatingFileHandler(
        os.path.join(logdir, filename + "_errors.log"), maxBytes=300000, backupCount=20
    )
