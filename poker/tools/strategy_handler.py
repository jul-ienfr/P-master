...
class StrategyHandler:
    ...
    def get_playable_strategy_list(self):
        ...
        lst = _http_post(
            URL + "get_playable_strategy_list",
    ...
    def check_defaults(self):
        ...
        if "RiverCheckDeceptionMinEquity" not in self.selected_strategy:
            self.selected_strategy["RiverCheckDeceptionMinEquity"] = 0.1
        if "TurnCheckDeceptionMinEquity" not in self.selected_strategy:
            self.selected_strategy["TurnCheckDeceptionMinEquity"] = 0.1
    ...
    def read_strategy(self, strategy_override=""):
        ...
        try:
            ...
            output = _http_post(
                URL + "get_strategy",
                params={"name": "Default", "login": "guest", "password": "guest"},
                timeout=10,
            ).json()[0]
    ...
    def save_strategy_genetic_algorithm(self):
        ...
        response = _http_post(
            URL + "save_strategy",
            json={
                "strategy": json.dumps(self.selected_strategy),
                "login": login,
                "password": password,
            },
            timeout=10,
    ...
    def save_strategy(self, strategy_dict):
        ...
        response = _http_post(
            URL + "save_strategy",
            json={"strategy": json.dumps(strategy_dict), "login": login, "password": password},
            timeout=10,
        ).json()
        if response:
            log.info("Saved")
