...
class TimesFM_2p5_200M_torch(
  ...
  def _from_pretrained(
    ...
    **model_kwargs,
    ...
    instance.model.load_checkpoint(
      model_file_path,
      torch_compile=instance.torch_compile,
    )
    return instance
  ...
  def compile(self, forecast_config: configs.ForecastConfig, **kwargs) -> None:
    ...
    if fc.max_context % self.model.p != 0:
      logging.info(
        "When compiling, max context needs to be multiple of the patch size %d. Using max context = %d instead.",
        self.model.p,
    ...
    if fc.max_horizon % self.model.o != 0:
      logging.info(
        "When compiling, max horizon needs to be multiple of the output patch size %d. Using max horizon = %d instead.",
        self.model.o,
    ...
    if fc.max_context + fc.max_horizon > self.model.config.context_limit:
      raise ValueError(
        ...
        f" {fc.max_context} + {fc.max_horizon} > {self.model.config.context_limit}."
      )
    ...
    def _compiled_decode(horizon, inputs, masks):
      ...
      inputs_t = torch.from_numpy(np.array(inputs)).to(self.model.device).to(torch.float32)
      masks_t = torch.from_numpy(np.array(masks)).to(self.model.device).to(torch.bool)
      ...
      if fc.normalize_inputs:
        ...
        inputs_t = revin(inputs_t, mu, sigma, reverse=False)
      else:
        mu, sigma = None, None

      pf_outputs, quantile_spreads, ar_outputs = self.model.decode(
        forecast_config.max_horizon,
        inputs_t,
        masks_t,
      )
      to_cat = [pf_outputs[:, -1, ...]]
