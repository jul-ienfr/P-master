...
class HistoryRoutesMixin:
    ...
    def _runtime_persistence_payload(history: dict, limit: int) -> dict:
        return HistoryRoutesMixin._history_bucket_payload(history.get("persisted", {}) or {}, limit)
    ...
    def _resolve_export_filename(export_format: str, stream: Optional[str]) -> str:
        ...
        if normalized in {"policy_compare", "policy-compare", "corpus", "bundle"}:
            return HistoryRoutesMixin._review_session_filename(stream)
        return HistoryRoutesMixin._export_filename(stream)
