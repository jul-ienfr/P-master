...
class SanityChecker:
    ...
    """
    def __init__(self):
        self.max_pot_allowed = 2000.0  # Sécurité hardcodée (ex: NL100, pot max théorique)
    ...
    def is_possible_new_hand_pot(self, old_pot: float, new_ocr_pot: float) -> bool:
        ...
        return new_ocr_pot > 0.0 and new_ocr_pot < old_pot and new_ocr_pot <= 5.0
    ...
    def evaluate_action_gate(
        ...
        if legal_actions and action_intent.action not in legal_actions:
            reasons.append(GateReason(
                code="ILLEGAL_ACTION",
    ...
    def validate_pot_evolution(self, old_pot: float, new_ocr_pot: float, total_bets: float) -> float:
        """
    ...
    def validate_stack_read(self, starting_stack: float, new_ocr_stack: float, current_bet: float) -> float:
        """
        ...
        if new_ocr_stack > starting_stack + 0.1: # +0.1 pour float rounding
            logger.warning(f"⚠️ Anomalie Stack bloquée ! Stack lu: {new_ocr_stack} > Stack de départ: {starting_stack}. L'OCR a halluciné des jetons.")
            return starting_stack - current_bet
