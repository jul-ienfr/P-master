...
fn mask_to_index(mut mask: u64, k: usize) -> usize {
    let mut index = 0;
    for table_row in COMB_TABLE.iter().take(k) {
        assert!(mask != 0);
        let tz = mask.trailing_zeros();
        index += table_row[tz as usize];
        mask &= mask - 1;
    }
    index
}
    ...
    fn phase2_process<const K: usize>(&mut self) {
        ...
        into_par_iter(start_index..end_index)
            ...
            .for_each(|chunk_start_index| {
                ...
                for freq_cell in &src_table[chunk_start_index..chunk_end_index] {
                    let mut src_mask_copy = src_mask;
                    src_mask = next_combination(src_mask);

                    let freq = freq_cell.load();
                    if freq == 0.0 {
                        continue;
                    }

                    let mut src_mask_bit = [0; K];
                    for slot in src_mask_bit.iter_mut().take(K) {
                        let lsb = src_mask_copy & src_mask_copy.wrapping_neg();
                        src_mask_copy ^= lsb;
                        *slot = lsb;
                    }

                    for (i, ones) in num_ones.iter().enumerate().take((1 << K) - 1) {
                        if *ones > 6 {
                            continue;
                        }

                        let mut dst_mask = 0;
                        for (j, bit) in src_mask_bit.iter().enumerate() {
                            if i & (1 << j) != 0 {
                                dst_mask |= bit;
                            }
                        }

                        let dst_index = mask_to_index(dst_mask, *ones as usize);
                        self.sum[*ones as usize][dst_index].add(freq);
                    }
                }
            });
    }
    ...
    fn phase3_process<const N: usize>(&mut self, start_index: usize, end_index: usize) {
        ...
        into_par_iter(start_index..end_index)
            ...
            .for_each(|dst_start_index| {
                ...
                for dst_cell in &dst_table[dst_start_index..dst_end_index] {
                    let mut mask_copy = mask;
                    mask = next_combination(mask);

                    let mut mask_bit = [0; N];
                    for slot in mask_bit.iter_mut().take(N) {
                    ...
                    for &(i, k) in &indices {
                        let mut src_mask = 0;
                        for (j, bit) in mask_bit.iter().enumerate() {
                            if i & (1 << j) != 0 {
                                src_mask |= bit;
                            }
                        }
                    ...
                    dst_cell.store(f32::max(result as f32, 0.0));
                }
            });
    }
}
