...
impl HandCategory {
    pub(crate) const ALL: [HandCategory; 9] = [
    ...
    #[inline]
    fn from_raw_value(raw_value: i32) -> Self {
        match raw_value >> 26 {
            ...
            value => panic!("invalid hand category: {value}"),
        }
    }
}
...
impl Hand {
    ...
    #[inline]
    pub(crate) fn evaluate_with_category(&self) -> (u16, HandCategory) {
        let raw_value = self.evaluate_internal();
        (
            HAND_TABLE.binary_search(&raw_value).unwrap() as u16,
            HandCategory::from_raw_value(raw_value),
        )
    }
