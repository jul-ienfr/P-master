...
from src.runtime.policy_compare import (
    build_empty_policy_compare_summary as _build_empty_policy_compare_summary_impl,
    build_policy_compare_summary as _build_policy_compare_summary_impl,
    build_runtime_ab_summary as _build_runtime_ab_summary_impl,
    compact_policy_compare_examples as _compact_policy_compare_examples_impl,
    dedupe_runtime_ab_decisions as _dedupe_runtime_ab_decisions_impl,
    extract_policy_compare_actions as _extract_policy_compare_actions_impl,
    extract_policy_compare_ev_by_action as _extract_policy_compare_ev_by_action_impl,
    extract_runtime_ab_decision as _extract_runtime_ab_decision_impl,
    normalize_runtime_action_name as _normalize_runtime_action_name_impl,
    normalize_runtime_street_name as _normalize_runtime_street_name_impl,
    policy_compare_sample_id as _policy_compare_sample_id_impl,
    policy_compare_spot_example as _policy_compare_spot_example_impl,
    policy_slug as _policy_slug_impl,
    runtime_ab_decision_key as _runtime_ab_decision_key_impl,
    safe_runtime_float as _safe_runtime_float_impl,
)
...
class SuperBotController:
    ...
    @staticmethod
    # Phase 2.7 — résumés policy-compare / A/B délégués à src/runtime/policy_compare.py.
    @staticmethod
    def _normalize_runtime_street_name(value: object) -> str:
        return _normalize_runtime_street_name_impl(value)

    @staticmethod
    def _normalize_runtime_action_name(value: object) -> str:
        return _normalize_runtime_action_name_impl(value)

    @staticmethod
    def _policy_slug(value: object, fallback: str = "runtime") -> str:
        return _policy_slug_impl(value, fallback)

    @staticmethod
    def _safe_runtime_float(value: object) -> Optional[float]:
        return _safe_runtime_float_impl(value)

    @classmethod
    def _extract_policy_compare_actions(cls, entry: dict) -> dict[str, str]:
        return _extract_policy_compare_actions_impl(entry)

    @classmethod
    def _extract_policy_compare_ev_by_action(cls, entry: dict) -> dict[str, float]:
        return _extract_policy_compare_ev_by_action_impl(entry)

    @staticmethod
    def _build_empty_policy_compare_summary() -> dict:
        return _build_empty_policy_compare_summary_impl()

    @classmethod
    def _policy_compare_sample_id(cls, entry: dict, fallback: str) -> str:
        return _policy_compare_sample_id_impl(entry, fallback)
    ...
    def _policy_compare_spot_example(
        ...
        return _policy_compare_spot_example_impl(
            entry, sample_id, baseline_action, challenger_action, ev_by_action
        )

    @staticmethod
    def _compact_policy_compare_examples(examples: list[dict], limit: int = 2) -> list[dict]:
        return _compact_policy_compare_examples_impl(examples, limit)

    def _build_policy_compare_summary(self, decisions: List[dict]) -> dict:
        return _build_policy_compare_summary_impl(decisions)

    @staticmethod
    def _extract_runtime_ab_decision(entry: dict) -> Optional[dict]:
        return _extract_runtime_ab_decision_impl(entry)

    @staticmethod
    def _runtime_ab_decision_key(entry: dict) -> Optional[tuple]:
        return _runtime_ab_decision_key_impl(entry)

    def _dedupe_runtime_ab_decisions(self, decisions: List[dict]) -> List[dict]:
        return _dedupe_runtime_ab_decisions_impl(decisions)

    def _build_runtime_ab_summary(self, decisions: List[dict]) -> dict:
        return _build_runtime_ab_summary_impl(decisions)
    ...
    # Phase 2.7 — géométrie table déléguée à src/vision/table_geometry.py.
    ...
    def _get_button_classifier(self) -> ButtonClassifier:
        """Phase 2.7 — classification déléguée à src/vision/button_classifier.py."""
