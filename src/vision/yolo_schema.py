            ...
            if child.startswith("-"):
                ...
                try:
                    ...
                    raise YoloDatasetSchemaError(
                        f"Clé names invalide dans le YAML YOLO: {child_key.strip()!r}"
                    ) from exc
...
def normalize_dataset_names(raw_names: Any) -> list[str]:
    ...
    if isinstance(raw_names, dict):
        try:
            ...
            return [
                str(raw_names[index] if index in raw_names else raw_names[str(index)])
                for index in ordered_indexes
            ]
        except (TypeError, ValueError, KeyError) as exc:
            raise YoloDatasetSchemaError(
                "Le champ names du YAML YOLO doit utiliser des index entiers."
            ) from exc
    raise YoloDatasetSchemaError(
        "Le champ names du YAML YOLO doit être une liste ou un dictionnaire index->nom."
    )
...
def validate_dataset_yaml_schema(
    ...
    *,
    ...
    if "nc" not in schema:
        raise YoloDatasetSchemaError(
            f"YAML dataset YOLO invalide ({data_yaml_path}): champ nc manquant."
        )
    if "names" not in schema:
        raise YoloDatasetSchemaError(
            f"YAML dataset YOLO invalide ({data_yaml_path}): champ names manquant."
        )
    ...
    try:
        ...
        raise YoloDatasetSchemaError(
            f"YAML dataset YOLO invalide ({data_yaml_path}): nc doit être un entier."
        ) from exc
    ...
    if names != expected_names:
        ...
        if first_mismatch is None:
            raise YoloDatasetSchemaError(
                f"Schéma YOLO incompatible pour {data_yaml_path}: ordre des classes invalide."
            )
        raise YoloDatasetSchemaError(
