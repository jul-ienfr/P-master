...
def test_classify_action_button_label_uses_ocr_keywords():
    ...
    assert (
        controller._classify_action_button_label(np.zeros((8, 8, 3), dtype=np.uint8), 1, 3)
        == "call_button"
    )
...
def test_classify_action_button_label_falls_back_to_geometry_for_two_buttons():
    ...
    assert (
        controller._classify_action_button_label(np.zeros((8, 8, 3), dtype=np.uint8), 0, 2)
        == "check_button"
    )
    assert (
        controller._classify_action_button_label(np.zeros((8, 8, 3), dtype=np.uint8), 1, 2)
        == "bet_button"
    )
...
def test_label_generic_action_buttons_relabels_and_sorts_buttons():
    controller = _make_controller()
    controller._classify_action_button_label = lambda crop, index, count: (
        "check_button" if index == 0 else "bet_button"
    )

    state = TableState(
        action_buttons=[
            DetectionResult(
                class_name="action_button_generic", confidence=0.9, bbox=(400, 700, 560, 790)
            ),
            DetectionResult(
                class_name="action_button_generic", confidence=0.9, bbox=(180, 700, 340, 790)
            ),
        ],
        metadata={},
    )
    ...
    assert [button.class_name for button in labeled_state.action_buttons] == [
        "bet_button",
        "check_button",
    ]
...
def test_stabilize_runtime_hero_cards_reuses_recent_cards_when_live_context_exists():
    ...
    try:
        state = TableState(
            action_buttons=[
                DetectionResult(class_name="check_button", confidence=0.9, bbox=(0, 0, 10, 10))
            ],
            metadata={"table_detected": True},
        )
        assert controller._stabilize_runtime_hero_cards((), ("7d", "2h", "Ad"), state) == (
            "Jd",
            "Td",
        )
    finally:
