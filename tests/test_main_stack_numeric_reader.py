...
def test_read_player_stack_accepts_plausible_stack_drop():
    controller = object.__new__(SuperBotController)
    controller.amount_ocr = types.SimpleNamespace(
...
def test_read_player_stack_keeps_previous_value_while_seat_is_quarantined():
    controller = object.__new__(SuperBotController)
    controller.amount_ocr = types.SimpleNamespace(
        read_and_parse_amount=lambda crop: 100.0,
        get_metadata=lambda: {
    ...
    controller.tracker = types.SimpleNamespace(
        sanity=types.SimpleNamespace(
            is_stack_read_quarantined=lambda seat_id: True,
            get_stack_read_quarantine_remaining=lambda seat_id: 1.5,
        ),
        players={},
    )
    controller.numeric_reader = None
    ...
    assert value == 5000.0
    assert metadata["skipped_due_to_quarantine"] is True
    assert metadata["selected_amount"] == 5000.0
    assert metadata["quarantine_remaining_s"] == 1.5
